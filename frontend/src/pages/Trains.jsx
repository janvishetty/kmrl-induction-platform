...
function RunNightlyButton({ onDone }) {
  const [running, setRunning] = useState(false);
  const [msg, setMsg] = useState("");

  const run = async () => {
    setRunning(true);
    setMsg("Crunching AI optimizer...");
    try {
      ...
      setMsg("Done! Reloading table...");
      // Automatically refresh the table after 1 second
      setTimeout(() => {
        if (onDone) onDone();
        setMsg("");
        setRunning(false);
  ...
  return (
    <div className="flex items-center gap-2 ml-4">
      ...
      >
        {running ? "Working..." : "Recompute Plan"}
      </button>
...
export default function Trains() {
  ...
  const serviceTrains = useMemo(() => {
    // If backend sends a clean service_list, use it and inject the status
    if (rawData && Array.isArray(rawData.service_list)) {
      return rawData.service_list.map((t) => ({ ...t, status: "SERVICE" }));
    }
      ...
      <PageHeader
        ...
        action={
          <div className="flex items-center gap-3">
            ...
            <RunNightlyButton onDone={() => trainPlanQ.refetch()} />
          </div>
        }
      />
