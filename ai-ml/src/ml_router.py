...
@router.get("/explain/{train_id}", response_model=ExplainResponse)
def explain_train(train_id: str):
    """Why is this train in SERVICE/STANDBY/IBL? (from Supabase)"""
    res = _sb.table("explanations").select("*").eq("train_id", train_id.upper()).order("plan_date", desc=True).limit(1).execute()
    if not res.data:
